layui.use(['form', 'layer', 'table', 'laytpl', 'laydate'], function () {
    ...
    $('body').on('click','.examineAll_btn',function(){
        ...
        $.post("/index.php/admin/buyer/star",{data:checkStatus.data,star:0},function (res) {
    ...
    $('body').on('click','.refundsALL_btn',function(){
        var checkStatus = table.checkStatus('userListTable'); //idTest 即为基础参数 id 对应的值
        var curr=$(".layui-laypage-em").next().html(); //当前页码值
        $.post("/index.php/admin/buyer/star",{data:checkStatus.data,star:1},function (res) {
            return  layer.msg(res.msg,{},function () {
                // 搜索条件
                var name = $("input[name=name]").val(); //姓名
                var phone = $("input[name=phone]").val(); //手机号
    ...
    var tableIns = table.render({
        ...
        cols: [[
            ...
            { field: 'type', title: '平台', minWidth: 50, width:60, align: "center" },
            { field: 'addressphone', title: '收货人手机号码', minWidth: 80, width:110, align: "center" },
            { field: 'wwid', title: 'ID', minWidth: 80, width:95, align: "center" },
