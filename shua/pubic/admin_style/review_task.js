layui.use(['form', 'layer', 'table', 'laytpl', 'laydate', 'upload'], function () {
    ...
    var task_number = function (d) {
        return '<p class="p2">'+d.user_task.task_number+'</p>';
    };
    ...
    var buywangwang = function (d) {
        return '<p class="p2">' + d.buyno_info.wwid + '</p>';
    }
    ...
    var userListBar = function (d) {
        if(d.state!="已上传"){
            return `<a class="layui-btn layui-btn-xs" lay-event="taskExamine">审核</a>
         <a class="layui-btn layui-btn-xs" lay-event="xiangqing">详情</a>`;
        }else{
            return `<a class="layui-btn layui-btn-normal layui-btn-xs" lay-event="setTop">返 款</a><a class="layui-btn layui-btn-xs" lay-event="taskExamine">审核</a>
         <a class="layui-btn layui-btn-xs" lay-event="xiangqing">详情</a>`;
        }
    }
    ...
    var tableIns = table.render({
        elem: '#userList',
        url: '/index.php/admin/task/review_task',
        method:'post',
        toolbar: '#toolbarDemo',
        //cellMinWidth: 95,
        page: true,
        ...
        cols: [[
            { type: 'checkbox' },
            { field: 'id', title: '任务编号id', width: 50, align: "center" },
            { field: 'task_number', title: '追评编号', width: 90, align: "center" },
            { field: 'task_number', title: '任务编号', width: 90, align: "center",templet:task_number },
            ...
            { field: 'money', title: '商家支付', minWidth: 50, width: 100, align: "center"},
            { field: 'user_money', title: '用户佣金', minWidth: 50, width: 100, align: "center"},
            { field: 'yjprice', title: '押金支付', minWidth: 50, width: 100, align: "center"},
            { field: 'ydprice', title: '银锭支付', minWidth: 50, width: 100, align: "center"},
            { field: 'create_time', title: '添加时间', minWidth: 50, width: 100, align: "center" },
            { field: 'confirm_time', title: '完成时间', minWidth: 50, width: 80, align: "center" },
    ...
    table.on('tool(userList)', function (obj) {
        ...
        if (layEvent === 'setTop') { //返款
            layer.confirm('确认返款?', function(index){
                $.post("/index.php/admin/task/reviewTaskReturnPay",{id:data.id},function (res) {
    ...
    $('body').on('click','.delAll_btn',function(){
        ...
        $.post("/index.php/admin/task/allexaminereview",{ids:ids},function (res) {
    ...
    $('body').on('click','.refundsAll_btn',function(){
        ...
        layer.confirm('确定批量返款？', {icon: 3, title: '提示信息'}, function (index) {
            $.post("/index.php/admin/task/reviewTaskReturnPays",{ids:ids},function (res) {
                return  layer.msg(res.msg,{},function () {
                    // 搜索条件
                    var task_number = $("input[name=task_number]").val(); //任务编号
                    var wangwang = $("input[name=wangwang]").val(); //买家旺旺ID
    ...
    form.on("submit(reload)", function (data) {
        ...
        var delivery_state = field.delivery_state; //发货状态
          var task_number_user = $("input[name=task_number_user]").val(); //发货状态
        table.reload("userListTable", {//搜索【此功能需要后台配合，所以暂时没有动态效果演示】
            ...
            where: {
                ...
                , delivery_state: delivery_state  //发货状态
                , time: time  //接单时间
                 ,task_number_user:task_number_user
            }
        })
        setTimeout(function () {
            top.layer.close(index);
    ...
    $('.video-rounded').click(function(){
        ...
        layer.open({
            ...
            content: '<div style="text-align:center"><video src='+img+' style="width:90%"/></video></div>'
        });
    })
    ...
    function taskExamine(id) {
        layui.layer.open({
            type: 2,
            content:"/index.php/admin/task/reviewTaskExamine/id/"+id,
            area: ['600px', '50%'],
            title: '任务审核',
            success : function(layero, index){
                var body = layui.layer.getChildFrame('body', index);
    ...
    function xiangqing(id) {
        layui.layer.open({
            type: 2,
            content:"/index.php/admin/task/reviewTaskXiangqing/id/"+id,
            area: ['80%', '80%'],
            title: '任务详情',
            success : function(layero, index){
                var body = layui.layer.getChildFrame('body', index);
