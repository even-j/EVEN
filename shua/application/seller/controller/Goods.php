...
class Goods extends Base
{
    ...
    {
        if(request()->isAjax()){
            $shop = Db::name('shop')->where(['seller_id'=>$this->seller['id'],'state'=>1])->field("id,shop_name,type")->select();
            $shop = $shop ? $shop->toArray() : [];
    ...
    public function edit(Request $request)
    {
        ...
        if(request()->isAjax()){
            $where['id'] = $data['id'];
            $shop = Db::name('shop')->where(['seller_id'=>$this->seller['id'],'state'=>1])->field("id,type,shop_name")->select();
            $shop = $shop ? $shop->toArray() : [];
            $shop_type=getShopType(1);
            foreach ($shop as &$item){
                $item['shop_name'] = $item['shop_name'].'('.$shop_type[$item['type']].')';
