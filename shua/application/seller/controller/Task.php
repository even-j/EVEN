...
$data['time'] = date('Y-m-d H:i:s');
            ...
            echo "<script>location='/seller/task/importList.html';</script>";
            die;
 ...
 public function importList(Request $request){
        if (request()->isPost()) {
            ...
            $total1 = Db::name('task_import')->where($where)->count('id');
            $list1 = Db::name('task_import')->where($where)->order('id desc')->limit($start,$data['size'])->select();
            if($list1)$list1 = $list1->toArray();
            $res['config'] = array(
                '0'=>"导入中",
                '1'=>"成功",
                '2'=>'失败(商品链接不存在)',
                '3'=>'店铺不存在','4'=>'整点任务与单数不符','5'=>'优质好评与单数不符','6'=>'商家余额不足','7'=>'商家余额不足'
            );
            foreach ($list1 as &$item){
                $item['state'] = $res['config'][$item['state'] ];
            }
    ...
    public function shop(Request $request){
        ...
        $shop = Db::name('shop')->where($where)->where(['state'=>1,'seller_id'=>$this->seller['id']])->field('id,type,shop_name')->select();
        $shop = $shop ? $shop->toArray() : [];

        $shop_type=getShopType(1);
        foreach ($shop as &$item){
            $item['shop_name'] = $item['shop_name'].'('.$shop_type[$item['type']].')';
    ...
    public function taskTowDo(Request $request){
       ...
       // if(!$data['is_free_shiping'])return $this->error('请选择是否包邮！');
        ...
        $edit = $this->handleData($data,$task_list['type']);
     ...
     * @return array 整理后数据
     */
    public function handleData($data,$comm_type=1){
        $divide_price = 0;
        ...
        /*套餐服务费计算*/
        $add['service_price'] = $this->service($goods_price,1,$comm_type); //套餐服务费
        /*返款服务费用计算*/
     ...
     * @throws \think\exception\DbException
     */
    public function service($price,$type=1,$comm_type=1){
        $res = 0;
        $list = Db::name('commission')->where('type',$comm_type)->where(['max_goods_price'=>['egt',$price]])->order('max_goods_price asc')->find();
        if($list){
    ...
    public function getData(Request $request){
        ...
        if(isset($data['pingtai']) && $data['pingtai']){
            $shop = Db::name('shop')->where(['type'=>$data['pingtai'], 'state'=>1,'seller_id'=>$this->seller['id']])->column('id');
        ...
        $complete_num = Db::name('seller_task')->where($where)->sum('complete_num');
        ...
        return $this->success('succress','',['list'=>$list,'total'=>$total,'complete_num'=>$complete_num]);
    }
    ...
    public function getUserTask(Request $request){
        ...
        if($data['shop_id'])$where['shop_id'] = $data['shop_id'];
        if($data['pingtai'])$where['type'] = $data['pingtai'];
        ...
        foreach ($list as &$item){
            $item['checked'] = false;
            $item['shop'] = Db::name('shop')->where(['id'=>$item['shop_id']])->value('shop_name').'('.$shop_type[$item['type']].')';
       ...
       b.video_praise_fee')->select();
        ...
        $system = Db::name('system')->find();
         foreach ($list as &$item){
            ...
            $goods_id = Db::name('task_goods')->where(['task_id'=>$task_id])->value('goods_id');
            $item['taobao_id']  =  Db::name('goods')->where(['id'=>$goods_id])->value('taobao_id');
        }


        $title = ['店铺名','任务编号','旺旺号','淘宝订单号','任务金额','付款金额','快递类型','快递单号','支付时间',
            '套餐服务费','返款服务费','文字优质好评','图片优质好评','视频优质好评','追评文字好评','追评图片好评','追评视频好评','淘宝商品id'];
    ...
    public function getTaskData(Request $request){
        ...
        $list['returnNum'] = $num;
        $list['commission'] = $this->service($list['goods_price'],2,$list['type']) + round($list['user_divided']/$list['num'],2);
    ...
    public function returnPay(Request $request){
       ...
       // $list = Db::name('user_task')->where(['id'=>$data['id'],'seller_id'=>$this->seller['id'],'state'=>5])->field("id,seller_task_id,task_number,principal,state")->find();
        $list = Db::name('user_task')->where(['id'=>$data['id'],'seller_id'=>$this->seller['id']])->field("id,user_id,seller_task_id,task_number,principal,state")->find();
