...
class Review extends Base
{
    ...
    public function getTaskData(Request $request){
        ...
        if($data['order_number'])$where['taobao_number'] = $data['order_number'];
        if($data['type']!=''){
    ...
    public function excel(Request $request){
        ...
        if(isset($data['search']) &&$data['search']) $where['task_number'] = $data['search'];
        if(isset($data['order_number']) &&$data['order_number'])$where['taobao_number'] = $data['order_number'];
        if(isset($data['type']) &&$data['type']){
            if(in_array($data['type'],['0','1','2','3','4','5','6']))
            $where['state'] = $data['type'];
        }
        ...
        $list = ReviewTask::where($where)->order('id desc')->select();
        $list = $list ? $list->toArray() : [];
        $data1 = $data =[];
        $typeArr = getShopType(1);

        foreach ($list as &$item){
            $goods= Db::name('task_goods')->where(['task_id'=>$item['task_id']])->field('goods_id,price,num,goods_spec,pc_img,name')->find();
            $data['type'] = $typeArr[$item['shop_id']['type']];
            $data['shop_name'] =$item['shop_id']['shop_name'];
            $data['task_number'] = $item['task_number'];
            $data['create_time'] = $item['create_time'];
            $data['buy_id'] = $item['buy_id'];
            $data['goods_name'] = $goods['name'];
            $data['state'] = $item['state'];
            $data['pay_price'] = $item['pay_price'];
            $data['taobao_number'] = $item['taobao_number'];
            $data['money'] = $item['money'];
            $data1[] = $data;
        }

        $title = ['平台','店铺','任务编号','发布时间','旺旺号','商品名','任务状态','买手支付金额','订单号','商家支付'];
        Phpexcel::exportExcel($title,$data1,'追评任务_'.date('Y-m-d'));
    }
