...
class My extends Base
{
    ...
    public function getTaskGoods($task_id){
        $key = md5('getTaskGoods::Find'.$task_id);
        $data = $this->redis->get($key);
        if(!$data){
            $data =  Db::name('task_goods')->where('task_id',$task_id)->find();
    ...
    public function getUserBuynoByUid($id){
        $key = md5('getUserBuynoByUid::'.$id);
        $data = $this->redis->get($key);
        if(!$data){
            $data = Db::name('user_buyno')
                ->where(['uid'=>$this->id])
                ->where(['state'=>1])
                ->select()->toArray();
    ...
    public function taskmanagement(Request $request){
        ...
        $buyno=$this->getUserBuynoByUid($this->id);
            ...
            if(!empty($data['zhuipin'])){
                ...
                foreach($list as $k =>&$v){
                    if($v['goods_id']){
                        ...
                        $task_goods[$k]= $this->getTaskGoods($v['seller_task_id']);// Db::name('task_goods')->where('task_id',$v['seller_task_id'])->find();
