...
class Task extends Base
{
    public function index(Request $request){
        ...
        if(request()->isAjax()) {
            ...
            if($data['buyno_id']){
                ...
                $userbuyno = $this->get_user_buyno($data['buyno_id']);
                if($userbuyno['type']<4){
                    $where['type'] = ['in',['1','2',3]];// $userbuyno['type'];
                }else{
                    $where['type'] = $userbuyno['type'];
                }


                $list = $this->getIndexData1($where);
                ...
                if ($panduanstar == 2) {
                    $listData = $this->getIndexData($where,$data['page']);
            ...
            foreach($list as $k =>&$v){
                ...
                $commission=$this->getCommission($v['total_price'],$v['type']);
                ...
                $shop_name=$this->getShopInfo($v['seller_id']);
        ...
        foreach ($buyno as $k=>$v){
            ...
            $count=4-$count;
            $buyno[$k]['count'] =$count;
            $buyno[$k]['wwid'] = $buyno[$k]['wwid'].'('.$shop_type[$v['type']].')';
        }
    ...
    public function getShopInfo($seller_id){
        $key = md5('getShopInfo:::'.$seller_id);
        $data = $this->redis->get($key);
        if(!$data){
            $data=Db::name('seller')
                ->where('id',$seller_id)
                ->find();
            $this->redis->set($key,json_encode($data),600);
        }else{
    ...
    public function getCommission($total_price,$type){
        $key = md5('getCommission:'.$total_price.'t:'.$type);
        $data = $this->redis->get($key);
        if(!$data){
            $data=Db::name('commission')->where('type',$type)
    ...
    public function getIndexData1($where){
        $key = md5('getIndexData1:'.json_encode($where));
        $data = $this->redis->get($key);
        if(!$data){
            $now=time();
            $data = SellerTask::where($where)
                ->where('publish_time', '<', $now)
                ->order('id desc')
                ->select();
            $this->redis->set($key,json_encode($data),5);
        }else{
            $data=json_decode($data,true);
        }
        return $data;
    ...
    public function getIndexData($where,$page){
         $key = md5('getIndexData:'.json_encode($where).$page);
        $data = $this->redis->get($key);

        if(!$data){

             //if($this->id ==2947)die('ss');
    ...
    public function get_user_bank_info($user_id){
        $key = md5('get_user_bank_info::'.$user_id);
        $data = $this->redis->get($key);
        if(!$data){
            $data = Db::name('user_bank')->where(['user_id'=>$user_id])->find();
    ...
    public function get_user_buyno($id){
        $key = md5('get_user_buyno::'.$id);
        $data = $this->redis->get($key);
        if(!$data){
            $data = Db::name('user_buyno')->where(['id' =>$id])->find();
            $this->redis->set($key,json_encode($data),86400);
        }else{
            $data=json_decode($data,true);
        }
    ...
    public function getTaskGoods($task_id){
        $key = md5('getTaskGoodsList:'.$task_id);
        $data = $this->redis->get($key);
        if(!$data){
            $data =  Db::name('task_goods')->where('task_id',$task_id)->select();
    ...
    public function get_task(Request $request)
    {
        ...
        $user_bank_info =$this->get_user_bank_info($this->id);//Db::name('user_bank')->where(['user_id'=>$this->id])->find();
        ...
        $r_key ='checkNums::'. $data['task_number'];
        if($this->redis->get($r_key)){
            return $this->error('此任务已全部被领取！');
        }

        $buyno = $this->get_user_buyno($data['buyno_id']);
        if($buyno['star']<1){
        ...
        $map['state'] = ['neq', 2];
        $count = Db::name('user_task')->where('seller_task_id',$seller_task['id'])->where($map)
            ->count();
        if($count>=$seller_task['num']){
            $this->redis->lrem($data['task_number'], $this->id, 0);
            $this->redis->set($r_key,1,600);
            return $this->error('此任务已全部被领取！');
        }
        ...
        //$res = $this->buyno($buyno['id'], $buyno['wwid'], $seller_task['seller_id'], $seller_task['shop_id'],$seller_task['id'],$buyno);
        $res = $this->buyno($seller_task,$buyno);
        if ($res['code'] == 0) {
            $this->redis->lrem($data['task_number'], $this->id, 0);
      ...
      /*  if ($buyno['star'] == 1 && $seller_task['goods_price'] > 100) {
        ...
        $task = [
            ...
            'code' => $seller_task['code'],'type' => $seller_task['type'],
        ];
        Db::startTrans();
        try {
            ...
            if ($seller_task['is_shengji'] == 2) {
                $task_goods = $this->getTaskGoods($seller_task['id']);// Db::name('task_goods')->where('task_id', $seller_task['id'])->select();
                $ids = [];
                $goods_ids = [];
...
**/
     ...
     * @throws \think\exception\DbException
     */
    //public function buyno($id,$wwid,$seller_id,$shop_id,$seller_task_id,$buyno=[]){
    public function buyno($seller_task,$buyno=[]){
        $id = $buyno['id'];
       // $wwid = $buyno['wwid'];
        $seller_id = $seller_task['seller_id'];
        $shop_id = $seller_task['shop_id'];
       // $seller_task_id = $seller_task['id'];
        ...
        $r_key2 ='buyno::'. $id.date('Y-m-d');
        if($this->redis->get($r_key2)){
            return ['code'=>0,'msg'=>'买号今天已接4单，达到上限！'];
        }

        $r_key1 ='buyno::'. $id.$seller_id.date('Y-m-d');
        if($this->redis->get($r_key1)){
            return ['code'=>0,'msg'=>'买号今天已接该商家任务，请明天继续！'];
        }

        $shopInfo = $this->getShopInfoById($shop_id);

        if($buyno['type'] !=$shopInfo['type']){
            if($buyno['type'] ==1){
                if($shopInfo['type']>3){
                    return ['code' => 0, 'msg' => '买号平台与任务平台不一致'];
                }
            }else{
                return ['code' => 0, 'msg' => '买号平台与任务平台不一致'];
            }
       ...
       // $shopInfo = Db::name('shop')->where('id',$shop_id)->find();
        ...
        if($list){
            $times = $seller_task['cycle_time'];// Db::name('seller_task')->where(['id'=>$seller_task_id])->value('cycle_time');
            $times = $times ? $times : 0;
            $shop_cycle_time = $shopInfo['cycle_time'];// Db::name('shop')->where(['id'=>$shop_id])->value('cycle_time');
    ...
    public function maketask(){
        $have_task=Db::name('user_task')
            ->field('b.id,a.shop_id,a.type,b.task_type,b.seller_id,b.state,b.terminal,a.user_divided,a.id,a.principal,a.commission,a.user_buyno_wangwang,a.seller_task_id,a.task_step,a.is_ys')
            ->alias('a')
        ...
        foreach($have_task as &$item){
            //$seller=Db::name('seller') ->where('id',$item['seller_id'])->find();
            $shop=$this->getShopInfoById($item['shop_id']);
            $item['seller']=substr_replace($shop['mobile'],'****',3,5);
            $item['type'] = $shop_type[$item['type']];
...
//        $this->assign('task_step',$task_step);
    ...
    public function del_task(){
        ...
        return $this->success('操作成功','mobile/task/maketask');
    }
    ...
    public function taskstep(){
        ...
        Db::name('user_task')->where('id',$params['id'])->update(['is_go'=>1]);    
        ...
        $end_time=$user_task['ending_time']*1000;
        ...
        $shop= $this->getShopInfoById($sell_task['shop_id']);

        $shop_type=getShopType(1);
        $shop['type_name'] = $shop_type[$shop['type']];
...
//        }
    ...
    public function convertUrlQuery($query="https://item.taobao.com/item.htm?id=603687399746")
    {
        ...
        if(strpos($query,'jd.com')){
            $pattern = '/\/([0-9]+)\.html$/';
            if (preg_match($pattern, $query, $matches)) {
                $id = $matches[1]; // $matches[1] 将包含 id 参数的值
                $result = array('id' => $id); // 将 id 值存储在数组中
                return $result;// 打印数组
            }
        }
        ...
        if(isset($params['goods_id']) && !isset($params['id'])){
...
//        }
        ...
        if($data['threeradio'] == 2){
            ...
            $user=Db::name('users')->where('id',$this->id)->find();
            $user['qualified']=$user['qualified']?$user['qualified']:0;
            if($user['qualified'] >= 5){
...
//        }
        ...
        if($user_task['ys_time']<=$now_zero || $user_task['ys_time']>=$now_two_four){
            return $this->error('不在预售时间之内，不能上传尾款截图。');
        }
        if($user_task['state'] !=0){
            return $this->error('此任务出现错误，请检查。');
        }
    ...
    public function confirm_refund(){
        ...
        if(empty($refund_task)){
            ...
            $value=4;
            return $this->success('返款成功',url('My/taskmanagement',['value'=>$value]));
        }
