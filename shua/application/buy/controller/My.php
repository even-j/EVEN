...
class My extends Base
{
    ...
    public function index(Request $request)
    {
        ...
        if(request()->isAjax()) {
            $buyno=Db::name('user_buyno')->where('uid',$this->id)->select()->toArray();
            ...
            if(!empty($data['zhuipin'])){
                ...
                $total = Db::name('user_task')
                    ->field('e.state,a.task_number,a.delivery_state,e.id,
                ...
                $list=Db::name('user_task')
                   ...
                   a.goods_id,a.user_buyno_id,a.principal,a.commission,a.user_divided,a.seller_principal,a.is_ys,a.yf_price,a.wk_price,
                ...
                foreach($list as $k =>&$v){
                    ...
                    if($v['is_ys']==1){
                        $v['seller_principal']=$v['yf_price'].+''.'+'.$v['wk_price'];
                    }
                    $shop_img=getShopType(3);
        ...
        $ejang = Db::name('user_reward_recharge')->where(['uid'=>$this->id,'type'=>20])->sum('price');
    ...
    public function detail(){
        $data=input('param.id');
        $list=Db::name('user_task')
            ->field('a.id,a.state,a.terminal,a.seller_task_id,a.task_number,a.create_time,a.seller_task_id,
        ...
        $img_praise=Db::name('seller_task_praise')->where(['id'=>['in',$ids],'type'=>2])->select();//value('content');
        $content =[];
        foreach ($img_praise as $item){
            $content = array_merge($content,json_decode($item['content']));
        }
        $list['img_praise']=$content;
    ...
    public function shoukuan(){
        ...
        $list=Db::name('user_task')
            ...
            a.video_praise,b.publish_time,b.task_type,c.wwid,d.shop_name,a.ids')
            ->alias('a')
            ->join('seller_task b','a.seller_task_id = b.id')
            ->join('user_buyno c','a.user_buyno_id = c.id')
            ->join('shop d','b.shop_id =d.id')
...
//        }
        ...
        $value=2;
        if($resa){
            return $this->success('确认收货成功',url('My/index',['value'=>$value]));
        }
    }
...
//            }
    ...
    public function add_buyno(){
        ...
        $buyno= Db::name('user_buyno')
            ...
            ->order('id DESC')
            ->field('type,wwid,id,creat_time,addressname,addresspro,addresscity,addressarea,addresstext,alipayimg,addressphone,state,note')
            ->select();
    ...
    public function addbuyno(){
        ...
        $buyno_count= Db::name('user_buyno')->where('type',$data['type'])
            ->where('uid',$this->id)
        ...
        if($buyno_count>=$check_nums){
            return $this->error('您最多只能添加'.$check_nums.'个买手号!');
        }
...
//        }
        ...
        if(!$data['provinceValue']){
            return $this->error('请输入请选择收货地址');
        }
        $addressphone=Db::name('user_buyno')->where($where_except_buyno)->where('type',$data['type'])->where('addressphone',$data['mobile'])->find();
        if($addressphone){
            return $this->error('此收货人手机号码已被注册。');
        }
        $addressphone=Db::name('user_buyno')->where($where_except_buyno)->where('type',$data['type'])->where('wwid',$data['wwid'])->find();
        if($addressphone){
            return $this->error('此ID已被注册。');
        }
        $code=session('code');
...
//            }
        ...
        $addressphone=Db::name('user_buyno')->where('id','<>',$data['id'])->where($where_except_buyno)->where('type',$data['type'])->where('addressphone',$data['mobile'])->find();
        if($addressphone){
            return $this->error('此收货人手机号码已被注册。');
        }
...
//        } //银行卡不能同名
        ...
        $message=[
            ...
            'create_time'=>time(),
            'idcard_img_a'=>$data['idcard_img_a'],//'http://tfkzpic.oss-cn-hangzhou.aliyuncs.com/'.$res,
            'idcard_img_b'=>$data['idcard_img_b'],//'http://tfkzpic.oss-cn-hangzhou.aliyuncs.com/'.$res2,
        ];
...
//            }
    ...
    public function check_mobile(){
        $mobile=input('mobile');$type = input('type');
        $where_except_buyno['state']=array('neq',3);
        $res=db('user_buyno')->where($where_except_buyno)
            ->where('type',$type)
            ->where('addressphone',$mobile)->find();
        if($res){
    ...
    public function edit_qq(){
        ...
        db('users')->where('id',$this->id)->update($newusername);
    ...
    public function edit_city1(){
        $data=input();
        $this->id =Session::get('id');//获取用户ID
        $newusername=[
            'city1'=>$data['city1'],'province1'=>$data['province1']
        ];
        db('users')->where('id',$this->id)->update($newusername);
        return $this->success('修改成功');
    }
