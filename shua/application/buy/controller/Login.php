...
class Login extends Controller//继承系统控制器
{
      ...
      public function autoTask(){
        ...
        if(!$system['auto_task']){
             echo $system['auto_task'];
            return;
        }
        $list =   db('seller_task')->where('status',2)->where('pay_state',1)->select()->toArray();
        foreach ( $list as $info){

            $data['id'] =$info['id'];
            $data['state']=3;
            $update_data['status']=3;
            $update_data['remarks']='自动审核';
            ...
            $insert_data['content'] = '淘宝任务审核';
            $insert_data['model'] = "计划任务自动审核:任务编号{$data_info['task_number']}";
            $insert_data['admin_id'] = 0;
    ...
    public function createTongJiDay($date){
          $start_time = strtotime($date);
          $end_time =$start_time+86399;
          $seller_task= $seller_recharge=$seller_cash=$user_recharge=$user_cash=$user_task=0;
          //商家任务
        $taskList = db('seller_task')->where('status',3)
              ->where('examine_time','between',[$start_time,$end_time])
              ->field('seller_id,num,is_hour_publish,hour_msg')
              //->group('seller_id')
              ->select()->toArray();

        $seller_arr =[];
        foreach ($taskList as $info){
            if($info['is_hour_publish']){
                $hour_msg = json_decode($info['hour_msg']);
                $info['num'] =array_sum($hour_msg);
            }
            $seller_task +=1;
            $user_task +=$info['num'];
            if(isset($seller_arr[$info['seller_id']])){
                $seller_arr[$info['seller_id']]['seller'] +=1;
                $seller_arr[$info['seller_id']]['user'] +=$info['num'];
            }else{
                $seller_arr[$info['seller_id']]['seller'] =1;
                $seller_arr[$info['seller_id']]['user'] =$info['num'];
            }
        ...
        foreach ($seller_arr as $k=>$v){
            $seller_task_tj=db('tongji')->where('seller_id',$k)->where('day',$date)->find();
            if($seller_task_tj){
                db('tongji')->where('id',$seller_task_tj['id'])->update(['seller_task'=>$v['seller'],'user_task'=>$v['user']]);
            }else{
                db('tongji')->insert(['seller_id'=>$k,'day'=>$date,'seller_task'=>$v['seller'],'user_task'=>$v['user']]);
            }
        }

        //商家存钱
        $seller_recharge = db('recharge')->where('state',1)->whereIn('pay_type',['1','3'])->where('user_type',1)
            ->where('create_time','between',[$start_time,$end_time])
            ->sum('price');

        //买家取钱
        $user_cash = db('user_cash')->where('state',3)//->whereIn('pay_type',['1','3'])->where('user_type',1)
            ->where('create_time','between',[$start_time,$end_time])
            ->sum('price');

        $day_task_tj=db('tongji')->where('seller_id',0)->where('day',$date)->find();
        if($day_task_tj){
            db('tongji')->where('id',$day_task_tj['id'])->update(
                ['seller_task'=>$seller_task,'user_task'=>$user_task,'seller_recharge'=>$seller_recharge,'user_cash'=>$user_cash]);
        }else{
            db('tongji')->insert(['seller_task'=>$seller_task,'user_task'=>$user_task,'seller_recharge'=>$seller_recharge,'user_cash'=>$user_cash,'day'=>$date]);
        }

    }

    public function seller_tj_d(){

            $taskList =  Db::name('seller_task')
                ->where('status',3)
                ->where('tj',0)
                ->select()->toArray();
            ...
            foreach ($taskList as $info){
                ...
                $m = date('Y-m',$info['create_time']);
                $seller_task_tj=db('seller_task_tj')->where('uid',$info['seller_id'])->where('month',$m)->find();
                if($seller_task_tj){
                    $seller_task_tj_data['task_nums'] = $seller_task_tj['task_nums']+1;
                    $seller_task_tj_data['user_task_nums'] = $seller_task_tj['task_nums']+$info['num'];
                    db('seller_task_tj')->where('id', $seller_task_tj['id'])->update($seller_task_tj_data);
                }else{
                    $seller_task_tj_data['uid'] = $info['seller_id'];
                    $seller_task_tj_data['month'] = $m;
                    $seller_task_tj_data['task_nums'] = 1;
                    $seller_task_tj_data['user_task_nums'] = $info['num'];
                    $seller_task_tj_data['time'] = strtotime(date('Y-m-01'));
                    db('seller_task_tj')->insert($seller_task_tj_data);
                }
                db('seller_task')->where('id', $info['id'])->update(['tj'=>1]);
            }

    }

    public function seller_tj_m1(){
        $list = Db::name('seller')->select()->toArray();
        ...
        foreach ($list as $info){
            $id = $info['id'];
            $taskList =  Db::name('seller_task')->where('examine_time','>=',$time)
                ->where('examine_time','<',$time2)
                ->where('seller_id',$id)->where('status',3)
                ->select()->toArray();
            $task_nums = count($taskList);
            $user_task_nums = 0;
            foreach ($taskList as $seller_task){
                if($seller_task['is_hour_publish']){
                    $hour_msg = json_decode($seller_task['hour_msg']);
                    $seller_task['num'] =array_sum($hour_msg);
                }
                $user_task_nums +=$seller_task['num'] ;
            }

            $insert_data['uid'] = $id;
            $insert_data['month'] = $m;
            $insert_data['task_nums'] = $task_nums;
            $insert_data['user_task_nums'] = $user_task_nums;
            $insert_data['time'] = $time;
            db('seller_task_tj')->insert($insert_data);
        }
    }

    public function seller_tj_m(){
        $list = Db::name('seller')->select()->toArray();
        $time =strtotime(date('Y-m-01'));
        $m = date('Y-m');
        //$time2 =$time+86400;
    ...
    public function check_tb_wangwang(){
        $list = Db::name('shop')
            ->where('type',1)//5
            ->where('tb_state',0)
...
// 以POST方式调用API
            ...
            if($result1->IsSuccess){
                ...
                Db::name('user_task')->where(['id'=>$info['id']])->update(['flag'=>2]);
            }
...
// 以POST方式调用API
    ...
    public function hour_task(){
        ...
        foreach ($list as $task) {
            ...
            if($task['num']<$nums){
                ...
                echo 'log::'.$task['id'].'  nums:'.$nums.' incomplete_num: '.$incomplete_num.'   ';
            }
        }
    ...
    public function channel_task(){
       ...
       $list = Db::name('user_task')
        ...
        $list1 = Db::name('user_task')->where('task_type',1)
        ->where('state', 0)->where('is_go','0')
        ...
        $list1 = Db::name('user_task')->where('task_type',2)
            ->where('state', 0)
            ->where('create_time','<', strtotime ( "-24 hour" ))
            ->select();
        $list1 = $list1 ? $list1->toArray() : [];
        if($list1){
            $list =array_merge($list,$list1);
            ...
            ->chunk(20,function ($list){
                ...
                foreach ($list as $info){
                    ...
                    if($goods_id){
                        ...
                        $list = Db::name('commission')->type('type',$shop['type'])->where(['max_goods_price'=>['egt',$goods_price]])->order('max_goods_price asc')->find();
                        if($list){
                        ...
                        $add['refund_service_price'] = 1;//$add['refund_service_price'] > 2 ? 2 : $add['refund_service_price'];
