    ...
    public function yingli(){
        if (request()->isPost()) {
            ...
            if (isset($date['username']) && $date['username']) {
                $where_name['seller_name'] = ['like', '%' . trim($date['username']) . '%'];
                $uid = db('seller')->where($where_name)->column('id');
                $where['seller_id'] = ['in',$uid];
            }else{
                $where['seller_id'] = 0;
            }
            ...
            if (isset($date['time']) && $date['time']) {
              ...
              //  $time1 = strtotime($time[0]);
               // $time2 = strtotime($time[1]);
                $where['day'] = ['between', [$time[0], $time[1]]];
            }else{
                $where['day'] = ['between', [date('Y-m-01'), date('Y-m-d')]];
                //return json(['code' => 0, 'count' => 0, 'msg' => '获取数据成功', 'data' => []]);
            }

            $list = db('tongji')->where($where)->order('day','asc')->select()->toArray();

            $all=['day'=>'统计','seller_recharge'=>0,'user_cash'=>0,'user_task'=>0];
            foreach ($list as $info){
                $all['seller_recharge'] +=$info['seller_recharge'];
                $all['user_cash'] +=$info['user_cash'];
                $all['user_task'] +=$info['user_task'];
            }
            $all['seller_recharge'] =round($all['seller_recharge'],2);
            $all['user_cash'] =round($all['user_cash'],2);
            $list[]=$all;

            return json(['code' => 0, 'count' => 0, 'msg' => '获取数据成功', 'data' => $list]);

            $task_id_arr = [];
            $data =[
                'seller_nums' =>0,'commission'=>0,'review_seller'=>0,'review_user'=>0
            ];
            $list = SellerTask::where($where)->where('pay_state',1)->select();
            foreach ($list as $item){
                $task_id_arr[]=$item['id'];
                $nums = $item['yajin']+$item['yinding'];
                $data['seller_nums'] =$data['seller_nums']+$nums;
            }

            $buyno_list = UserTask::whereIn('seller_task_id',$task_id_arr)->where('state',1)->select();

            foreach ($buyno_list as $item){
                $nums = $item['commission']+$item['user_divided'];
                $data['commission'] =$data['commission']+$nums;
            }

            $review = ReviewTask::whereIn('task_id',$task_id_arr)->where('state',2)->select();
            foreach ($review as $item){
                $data['review_seller'] =$data['review_seller']+$item['money'];
                $data['review_user'] =$data['review_user']+$item['user_money'];
            }
            $data['moeny']= $data['seller_nums']+ $data['review_seller']- $data['commission']- $data['review_user'];
            foreach ($data as  &$v){
                $v =round($v,2);
            }
            $data1[] =$data;

            return json(['code' => 0, 'count' => 0, 'msg' => '获取数据成功', 'data' => $data1]);
        }
        return view('PlatformCounterpayment/yingli');
    }
...
//            }
            ...
            if (isset($date['type']) && $date['type']) {
                $where['tfkz_user_task.type'] =$date['type'];
            }
...
//        }
 ...
 public function return_price111()
    {
        $data=input();
        ...
        if($user_task['principal']-100>$data['price'] || $user_task['principal']+100<$data['price']){
            return $this->error('买家自填本金上下浮动不能超过100元！');
        }

        if($user_task['user_principal'] == $data['price']){
            return   $this->error('金额错误！');
        }

        if($user_task['state']==3 && $user_task['delivery_state']==1) {
            Db::startTrans();
            $res = db('user_task')->where('id', $data['taskid'])->update(['user_principal' => $data['price']]);
            ...
            return $this->error('任务状态不正确，只有待发货才能修改！');
        }
        return  $this->success('修改成功！');
    }
    ...
    public function return_price()
    {
        ...
        if($user_task['seller_principal'] == $data['price']){
            return   $this->error('金额错误！');
        }

        if(!($user_task['state']==5|| $user_task['state']==6||$user_task['state']==4||$user_task['state']==3))return $this->error('任务状态不正确，只有待发货,待收货,待返款才能修改！');
        ...
        if($user_task['state']==6){
            $seller = Db::name('seller')->where('id',$user_task['seller_id'])->find();
            $userA=Db::name('users')->where('id',$user_task['user_id'])->find();
            if($user_task['seller_principal'] > $data['price']){
                $dif_price = $user_task['seller_principal']-$data['price'] ;
                if($userA['balance'] < $dif_price){
                    return $this->error('买家余额不足！');
                }

                Db::name('users')->where('id', $user_task['user_id'])->setDec('balance',$dif_price);
                if(!finance($user_task['user_id'], 2, -$dif_price, 1, 7,"任务{$user_task['task_number']}后台修改返款本金,扣除本金{$dif_price}元"))throw new Exception('财务写入失败！');

                Db::name('seller')->where('id', $user_task['seller_id'])->setInc('balance',$dif_price);
                if(!finance($seller['id'],1, $dif_price,1,9,"任务{$user_task['task_number']}后台修改返款本金,退还本金{$dif_price}元"))throw new Exception('财务写入失败！');
            }else{
                //数字大了， 扣商家给买家
                $dif_price = $data['price'] - $user_task['seller_principal'];
                if($seller['balance'] < $dif_price){
                    return $this->error('商家余额不足！');
                }

                Db::name('users')->where('id', $user_task['user_id'])->setInc('balance',$dif_price);
                if(!finance($user_task['user_id'], 2, +$dif_price, 1, 7,"任务{$user_task['task_number']}后台修改返款本金,退还本金{$dif_price}元"))throw new Exception('财务写入失败！');

                Db::name('seller')->where('id', $user_task['seller_id'])->setDec('balance',$dif_price);
                if(!finance($seller['id'],1, -$dif_price,1,9,"任务{$user_task['task_number']}后台修改返款本金,扣除本金{$dif_price}元"))throw new Exception('财务写入失败！');
            }
        }
...
//                $where['user_task_id']=['in',$shopid];
            ...
            if (isset($date['task_number_user']) && $date['task_number_user']) {
                $where111['task_number'] = ['like', '%' . trim($date['task_number_user']) . '%'];
                $task_number_arr=db('user_task')->where($where111)->column('id');
                $where['user_task_id']=['in',$task_number_arr];
            }
...
//        }
        ...
        if($data_info['state']==7){
            return $this->error('已拒绝的订单不能操作！');
        }
        ...
        try{
            ...
            if($data['state']==7){
                ...
                $seller = db('seller')->where('id',$data_info['seller_id'])->find();
                $shop = Db::name('shop')->where(['id'=>$data_info['shop_id']])->value('shop_name');
                $seller_edit = [];
                if($data_info['yjprice'] > 0)$seller_edit['balance'] = $seller['balance'] + $data_info['yjprice'];
                if($data_info['ydprice'] > 0)$seller_edit['reward'] = $seller['reward'] + $data_info['ydprice'];
                Db::name('seller')->where(['id'=>$seller['id']])->update($seller_edit);
                if($data_info['ydprice'] > 0){
                    if(!finance($seller['id'],1,$data_info['ydprice'],2,17,"拒绝《{$shop}》店铺追评任务{$data_info['task_number']}退回押金{$data_info['ydprice']}元"))throw new Exception('银锭退回财务写入失败');
                }
                if($data_info['yjprice'] > 0){
                    if(!finance($seller['id'],1,$data_info['yjprice'],1,17,"拒绝《{$shop}》店铺追评任务{$data_info['task_number']}退回本金{$data_info['yjprice']}元"))throw new Exception('押金退回财务写入失败！');
                }
