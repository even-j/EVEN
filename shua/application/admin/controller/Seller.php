...
class Seller extends Base
{
    ...
    public function BuyerList()
    {
        if(request()->isPost()) {
            ...
            $uid_arr = array_column($buy_list,'id');


            $m = date('Y-m-01');
            $m1 = date('Y-m-01',strtotime('-1 month', time()));
            $seller_task_tj = $seller_task_tj1= [];
            $list =    db('tongji')->where('day','>=',$m1)->where('day','<',$m)
                ->whereIn('seller_id',$uid_arr)
                ->field('seller_id,sum(user_task) as user_task')
                ->group('seller_id')
                ->select()->toArray();


            foreach ($list as $value){
                $seller_task_tj1[$value['seller_id']]['task_nums']=$value['user_task'];

            }
            $list =    db('tongji')->where('day','>=',$m)
                ->whereIn('seller_id',$uid_arr)
                ->field('seller_id,sum(user_task) as user_task')
                ->group('seller_id')
                ->select()->toArray();
            foreach ($list as $value){
                $seller_task_tj[$value['seller_id']]['task_nums']=$value['user_task'];

            }
            foreach ($buy_list as &$item){
                $item['task_nums1'] = isset($seller_task_tj1[$item['id']])?$seller_task_tj1[$item['id']]['task_nums']:0;
              //  $item['user_task_nums1'] = isset($seller_task_tj1[$item['id']])?$seller_task_tj1[$item['id']]['user_task_nums']:0;
                $item['task_nums'] = isset($seller_task_tj[$item['id']])?$seller_task_tj[$item['id']]['task_nums']:0;
               // $item['user_task_nums'] = isset($seller_task_tj[$item['id']])?$seller_task_tj[$item['id']]['user_task_nums']:0;
            }
