...
/**
 * 大金额格式化
 * 如果整数位不超过5位，那么直接按照普通的金额格式化，丢掉小数部分；如果整数位大于等于5位，那么就以万为单位，小数保留两位进行格式化
 * @param myMoney
 */
function bigMoneyFormat(myMoney){
	if(myMoney == "" || myMoney == null){
		return "0";
	}
	myMoney = myMoney.toString();
	var pointIndex = myMoney.indexOf(".");
	var length = myMoney.length;
	var leftLength = length;
	if(pointIndex != -1){
		leftLength = pointIndex;
	}
	if(leftLength < 5){
		return formatYuan(myMoney);
	}else{
		return myRoundCelling(accDiv(myMoney,10000), 2) + "万";
	}
}
...
function layeralert(text,btn,callback) {
    if(btn == null || btn == ''){
        btn = '确定';
    }
    layer.open({
...
//询问框
function layerconfirm(text,btn, callback) {
    if(btn == null){
        btn = ['确定', '取消'];
    }
    layer.open({
