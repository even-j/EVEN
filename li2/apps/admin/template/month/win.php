...
<div class="container clearfix">
        ...
        <div class="search-wrap">
            <div class="search-content">
                <form action="/index.php" method="get">
                    ...
                    <input type="hidden" name="ac" value="win" >
                        ...
                        <tr>
                            ...
                            <td>
                                <select name="status" id="status" class="common-select">
                                    ...
                                    <option <?php echo $condition['status'] == '3'?'selected="selected"':''?> value="3">申请结束</option>
                ...
                <div id="div_content" class="result-content" style="width:100%;overflow: auto">
                    <table class="result-tab" style="width:100%" >
                        <tr>
                            ...
                            <th style="width: 80px;">证券账户</th>
                            <th style="width: 100px;">实盘金额<br>(万元)</th>
                            <th style="width: 60px;">倍数</th>
                            <th style="width: 80px;">补亏金额<br>(元)</th>
                            <th style="width: 100px;">证券总资产<br>(元)</th>
                            <th style="width: 100px;">更新时间</th>
                            <th style="width: 80px;">总盈亏<br>(元)</th>
                            <th style="width: 80px;">保证金<br>(元)</th>
                            <th style="width: 100px;">剩余保证金<br>(元)</th>
                        ...
                        <?php foreach ($list as $item){?>
                            <tr>
                                ...
                                <td><?php echo date('m-d H:i',$item['update_time']) ?></td>
                                <td><?php echo floatval($item['fill_loss_money'])/100 ?></td>
                                <td><?php echo floatval($item['bond_total'])/100 ?></td>
                                <td><?php echo floatval($item['back_money'])/100 ?></td>
                                <td><?php echo date('Ymd',$item['pz_time']).$item['pz_id'] ?></td>
                                <td><?php echo $item['status']==2? \Model\Sys\Common::countdown($item['end_time']) :($item['status']==3?'<span style="color:red">申请结束</span>':'已结束') ?></td>
                                <td>
                                    <?php echo '<a href="javascript:huabo('.$item['pz_id'].')">编辑</a>' ?>
                                    &nbsp;&nbsp;<?php if($item['status']==3){?><a class="link-update" href="javascript:returnCaopan(<?php echo $item['pz_id'];?>)">退回请求</a><?php }?>
                                </td>
                            </tr>
                        <?php }?>             
                    </table>
    ...
    <script>
        function huabo(pz_id){
            window.location.href = '/index.php?app=admin&mod=month&ac=winedit&pz_id='+pz_id;
        }
        function returnCaopan(pz_id){
            window.location.href = '/index.php?app=admin&mod=peizi&ac=doWinCancel&pz_id='+pz_id;
        }
    </script>
    <!--/main-->
</div>
