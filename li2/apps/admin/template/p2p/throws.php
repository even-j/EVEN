...
<div class="container clearfix">
        ...
        <div class="search-wrap">
            <div class="search-content">
                <form action="/index.php" method="get">
                    ...
                    <input type="hidden" name="ac" value="throws" >
                        ...
                        <tr>
                            ...
                            <td>
                                <select name="status" id="status" class="common-select">
                                    ...
                                    <option <?php echo $condition['status'] == '2'?'selected="selected"':''?> value="2">未过期</option>
                                    <option <?php echo $condition['status'] == '3'?'selected="selected"':''?> value="3">已过期</option>
                                </select>
                            </td>
                            <th width="80">策略时间:</th>
                ...
                <div id="div_content" class="result-content" style="width:100%;overflow: auto">
                    <table class="result-tab" style="width:100%" >
                        <tr>
                            ...
                            <th style="width: 80px;">完成进度<br>(%)</th>
                            <th style="width: 110px;">投标开始时间</th>
                            <th style="width: 110px;">投标结束时间</th>
                        ...
                        <?php foreach ($list as $item){?>
                            <tr>
                                ...
                                <td><?php echo round($item['has_touzi_money']/$item['pz_money']*100,2) ?></td>
                                <td><?php echo empty($item['limit_start_time'])?'':date('m-d H:i',$item['limit_start_time']) ?></td>
                                <td><?php echo empty($item['limit_end_time'])?'':date('m-d H:i',$item['limit_end_time']) ?></td>
                                <td><?php echo $item['p2pstatus']==3?'已废标':(time()>$item['limit_end_time'] ?'<span style="color:red">过期</span>': \Model\Sys\Common::countdown($item['limit_end_time'])) ?></td>
    ...
    <script>
        function huabo(pz_id){
            window.location.href = '/index.php?app=admin&mod=p2p&ac=throwsedit&pz_id='+pz_id;
        }
    </script>
    <!--/main-->
</div>
