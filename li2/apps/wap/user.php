...
class user extends \apps\UserControl {
    ...
    public function account() {
        ...
        $keywords = isset($data) && $data['tags'] ? $data['tags'] : $data['name'] . ',' . SITE_TITLE;
        $description = isset($data) && $data['description'] ? $data['description'] : SITE_TITLE . $data['name'];
        ...
        $user['peizi_count'] = \Model\User\UserInfo::get_peizi_count($uid);
        $user['is_bind_bankcard'] = 0;
        $user['is_bind_idcard'] = empty($user ['id_card'])?0:1;
        $bank_card = \Model\User\BankCard::getByUid($uid);
        if ($bank_card) {
            $user['is_bind_bankcard'] = 1;
...
//        }
        ...
        $order = 'fund_id desc';
        $rows = \Model\Sys\Common::getTableRrcordList('user_fund_record r ', '*', $where, $order, $curpage, $pagesize);
        $rowcount = \Model\Sys\Common::getTableRecordCount('user_fund_record r', $where);
    ...
    public function fund_data() {
        $fundtype = isset($_POST ['fundtype']) ? intval($_POST ['fundtype']) : 0;
        $curpage = isset($_POST ['page']) ? intval($_POST ['page']) : 1;
        $pagesize = 10;
        $where = 'r.uid=' . $this->uid;
        if ($fundtype > 0) {
            $where .= ' and type=' . $fundtype;
    ...
    public function recharge() {
        ...
        $site_pay = \Model\Admin\Params::get('site_pay');
        //$this->assign('pay_start', $site_pay['pay_start']);
        $this->assign('pay_bus', $site_pay['pay_bus']);
        $this->assign('pay_bus_wx', $site_pay['pay_bus_wx']);
        $this->assign('pay_bus_zfb', $site_pay['pay_bus_zfb']);
    ...
    public function recharge_iframe() {
        $this->setTitle('充值—' . SITE_TITLE);
        $url = isset($_GET ['url']) ? \App::t($_GET ['url']) : '';
        $this->assign('url', $url);
        $this->template('recharge_iframe.php');
    }
    ...
    public function recharge_pay()
    {
        $this->setTitle('用户充值—' . SITE_TITLE);
        $money = isset($_GET ['money']) ? floatval($_GET ['money']) : '';
        $type= isset($_GET ['type']) ? $_GET ['type'] : 'offline';  
        $id= isset($_GET ['id']) ? $_GET ['id']:'';
        
        if($type=='offline') //线下
        {
            $id=intval($id);
            $row=\Model\Pay\PaySet::getAccount($id);
        }
        else {
            $row= \Model\Pay\PaySet::getSet($id);
        }
        $this->assign('type',$type);
        $this->assign('row',$row);
        $this->assign('uid', $this->uid);
        $this->template('recharge_pay.php');
    }
    ...
    public function recharge_bank() {
        ...
        $this->assign('uid', $this->uid);
        $this->template('recharge_bank.php');
    }
    //充值
    ...
    //充值
    public function recharge_alipaysm() {
        $this->setTitle('用户充值—' . SITE_TITLE);

        $this->assign('uid', $this->uid);
        $this->template('recharge_alipaysm.php');
    }
    //充值
    public function recharge_weixinsm() {
        $this->setTitle('用户充值—' . SITE_TITLE);

        $this->assign('uid', $this->uid);
        $this->template('recharge_weixinsm.php');
    }
    ...
    public function withdrawlMoneyFirst() {
        ...
        if ($_POST) {
            ...
            if (!$card_id) {
                $this->fontRedirect('请选择要提现的银行卡', \App::URL('wap/user/withdrawl'));
            }
            ...
            $this->fontRedirect('提现金额未填写', \App::URL('wap/user/withdrawl'));
        }
    }
    ...
    public function withdrawalsMoney() {
        $uid = $this->uid;
        if ($_POST && $uid > 0) {
            $url = \App::URL('wap/user/withdrawl');
    ...
    public function txok() {
        ...
        $message ['first_link'] = '<a href="' . \App::URL('wap/user/fund') . '" class="submit">资金管理</a>';
        $message ['second_link'] = '<a href="' . \App::URL('wap/user/account') . '" style="padding-left:27px;"> 返回账号设置</a> ';
    ...
    public function bank() {
        ...
        if (!$user ['id_card'] && !$user ['true_name']) {
            $this->fontRedirect('您的身份信息未绑定!', \App::URL('wap/user/sfz'));
        }
        $bankinfo = array();
    ...
    public function bankinfoveried() {
        if ($_POST) {
            ...
            $card_no = isset($_POST ['number']) ? preg_replace('/[\s]/', '', \App::t($_POST ['number'])) : '';
            ...
            if (\Model\User\BankCard::checkBandCardIsExist($card_no)) {
                exit(json_encode(array('code'=>0,'msg'=>'该卡号已存在，不能重复添加!')));
    ...
    public function sfzok() {
        ...
        $message ['first_link'] = '<a href="' . \App::URL('wap/user/bank') . '" class="submit">绑定银行卡</a>';
        $message ['second_link'] = '<a href="' . \App::URL('wap/user/account') . '" style="padding-left:27px;"> 返回账号设置</a> ';
        $this->assign('message', $message);
        $this->template('success.php');
    }
    ...
    public function passwordok() {
        ...
        $message ['first_link'] = '<a href="' . \App::URL('wap/member/login') . '" class="submit">立即登录</a>';
        $message ['second_link'] = '<a href="' . \App::URL('wap/member/account') . '" style="padding-left:27px;"> 返回账号设置</a> ';
        $this->assign('message', $message);
        $this->template('success.php');
    }
    ...
    public function peizi() {
        ...
        $pagesize = 3;
        $where = 'pz_type=' . $pz_type . ' and status>0 and uid=' . $this->uid;
        if(!empty($num)){
    ...
    public function peizi_data() {
        $pz_type = isset($_POST ['pz_type']) ? intval($_POST ['pz_type']) : 1;
        $curpage = isset($_POST ['page']) ? intval($_POST ['page']) : 1;
        $num = isset($_POST ['num']) ? strtoupper($_POST ['num'])  : '';//方案号
        $sp_user = isset($_POST ['sp_user']) ? $_POST ['sp_user'] : '';//操盘账号
        $pagesize = 3;
        $where = 'pz_type=' . $pz_type . ' and status>0 and uid=' . $this->uid;
        if(!empty($num)){
            $where .= ' and pz_id='.intval(substr($num,8));
    ...
    public function peizi_detail_data() {
        $pz_id = isset($_GET ['pz_id']) ? intval($_GET ['pz_id']) : 0;
        $curpage = isset($_GET ['page']) ? intval($_GET ['page']) : 1;
        $pagesize = 10;
        $where = "uid=" . $this->uid . " and table_name='user_peizi' and rec_id=" . $pz_id;
        ...
        $this->assign('fund_rows', $fund_rows);
        $this->template('peizi_detail_data.php');
    }
    //历史实盘信息
    ...
    public function p2p_touzi_detail() {
        ...
        if (!$data) {
            $this->fontRedirect('该投资单号不存在', \App::URL('wap/user/p2p_touzi'));
        }
        $this->assign('data', $data);
    ...
    public function tuiguang_user() {
        ...
        $order = ' uid desc';
        $rows = \Model\Sys\Common::getTableRrcordList('user_info ', '*', $where, $order, $curpage, $pagesize);
        $rowcount = \Model\Sys\Common::getTableRecordCount('user_info', $where);
        $var['rowcount'] = $rowcount;
        $var['pagesize'] = $pagesize;
        $this->assign('var',$var);
    ...
    public function tuiguang_user_data() {
        $curpage = isset($_POST ['page']) ? intval($_POST ['page']) : 1;
        $search_param['begindate'] = isset($_POST ['begindate']) ? $_POST ['begindate'] : '';
        $search_param['enddate'] = isset($_POST ['enddate']) ? $_POST ['enddate'] : '';
        $pagesize = 15;
        $where = ' introducer_id=' . $this->uid;
        if(!empty($search_param['begindate']))
        {
        ...
        $rows = \Model\Sys\Common::getTableRrcordList('user_info ', '*', $where, $order, $curpage, $pagesize);
        $this->assign('rows', $rows);
        $this->template('tuiguang_user_data.php');
    }
    //充值记录
    public function tuiguang_recharge() {
        ...
        $curpage = isset($_GET ['page']) ? intval($_GET ['page']) : 1;
        $pagesize = 15;
        $where = ' r.status=1 and r.uid in ('. implode(',', $uids).')';
        if(!empty($search_param['begindate']))
        {
        ...
        $order = ' r.rtime desc';
        //$rows = \Model\Sys\Common::getTableRrcordList('user_recharge_offline r left join user_info u on r.uid=u.uid ', "r.add_time,r.money,u.mobile,u.true_name", $where, $order, $curpage, $pagesize);
        $rows = \Model\Sys\Common::getTableRrcordList('v_recharge_record r left join user_info u on r.uid=u.uid ', "r.rtime,r.money,u.mobile,u.true_name", $where, $order, $curpage, $pagesize);
        $total = \Model\Sys\Common::getTableRrcordList('v_recharge_record r left join user_info u on r.uid=u.uid ', "'',sum(r.money) money,'合计' as mobile,''", $where, $order, $curpage, $pagesize);
        $rowcount = \Model\Sys\Common::getTableRecordCount('v_recharge_record r left join user_info u on r.uid=u.uid ', $where);
        $var['rowcount'] = $rowcount;
        $var['pagesize'] = $pagesize;
        $this->assign('var',$var);
        $this->assign('user', $this->user);
    ...
    public function tuiguang_recharge_data() {
        $users = \Common\Query::select('user_info', array('introducer_id'=>$this->uid), array('uid'));
        $search_param['begindate'] = isset($_POST ['begindate']) ? $_POST ['begindate'] : '';
        $search_param['enddate'] = isset($_POST ['enddate']) ? $_POST ['enddate'] : '';
        $uids = array();
        ...
        $rows = \Model\Sys\Common::getTableRrcordList('v_recharge_record r left join user_info u on r.uid=u.uid ', "r.rtime,r.money,u.mobile,u.true_name", $where, $order, $curpage, $pagesize);
        $this->assign('rows', $rows);
        $this->template('tuiguang_recharge_data.php');
    }
    ...
    public function tuiguang_withdraw() {
        ...
        $order = ' r.rtime desc';
        //$rows = \Model\Sys\Common::getTableRrcordList('user_recharge_offline r left join user_info u on r.uid=u.uid ', "r.add_time,r.money,u.mobile,u.true_name", $where, $order, $curpage, $pagesize);
        $rows = \Model\Sys\Common::getTableRrcordList('user_withdraw_record r left join user_info u on r.uid=u.uid ', "u.true_name,r.rtime,r.money,u.mobile", $where, $order, $curpage, $pagesize);
        $total = \Model\Sys\Common::getTableRrcordList('user_withdraw_record r left join user_info u on r.uid=u.uid ', "'' as true_name,'',sum(r.money) money,'合计' mobile", $where, $order, $curpage, $pagesize);
        $rowcount = \Model\Sys\Common::getTableRecordCount('v_recharge_record r left join user_info u on r.uid=u.uid ', $where);
        $var['rowcount'] = $rowcount;
        $var['pagesize'] = $pagesize;
        $this->assign('var',$var);
    ...
    public function tuiguang_withdraw_data() {
        $users = \Common\Query::select('user_info', array('introducer_id'=>$this->uid), array('uid'));
        $search_param['begindate'] = isset($_POST ['begindate']) ? $_POST ['begindate'] : '';
        $search_param['enddate'] = isset($_POST ['enddate']) ? $_POST ['enddate'] : '';
        $uids = array();
        ...
        $rows = \Model\Sys\Common::getTableRrcordList('user_withdraw_record r left join user_info u on r.uid=u.uid ', "u.true_name,r.rtime,r.money,u.mobile", $where, $order, $curpage, $pagesize);
        $this->assign('rows', $rows);
        $this->template('tuiguang_withdraw_data.php');
    }
    ...
    public function tuiguang_peizi() {
        ...
        $curpage = isset($_GET ['page']) ? intval($_GET ['page']) : 1;
        $pagesize = 1;
        $where = ' pz.pz_type in(1,2) and pz.uid in ('. implode(',', $uids).')';
        if(!empty($search_param['begindate']))
        {
        ...
        $order = ' pz.pz_id desc';
        $rows = \Model\Sys\Common::getTableRrcordList('user_peizi pz left join user_info u on pz.uid=u.uid', "pz.pz_id,pz.pz_type,pz.pz_times,pz.pz_time,pz.bond_total,pz.pz_ratio,pz.manage_cost_day,pz.status,pz.profit_loss_money,u.mobile,u.true_name", $where, $order, $curpage, $pagesize);
        $total = \Model\Sys\Common::getTableRrcordList('user_peizi pz left join user_info u on pz.uid=u.uid', "'','','','',sum(pz.bond_total*pz.pz_ratio) as  bond_total,'','','',sum(pz.manage_cost_day) as manage_cost_day,sum(pz.profit_loss_money) as profit_loss_money,'合计' as mobile,''", $where, $order, $curpage, $pagesize);

        $rowcount = \Model\Sys\Common::getTableRecordCount('user_peizi pz left join user_info u on pz.uid=u.uid', $where);
        $var['rowcount'] = $rowcount;
        $var['pagesize'] = $pagesize;
    ...
    public function tuiguang_peizi_data() {
        $users = \Common\Query::select('user_info', array('introducer_id'=>$this->uid), array('uid'));
        $search_param['begindate'] = isset($_POST ['begindate']) ? $_POST ['begindate'] : '';
        $search_param['enddate'] = isset($_POST ['enddate']) ? $_POST ['enddate'] : '';
        $search_param['status'] = isset($_POST ['status']) ? $_POST ['status'] : '';
        $uids = array();
        if($users){
            foreach ($users as $user) {
                $uids[] = $user['uid'];
        ...
        if(!empty($search_param['enddate']))
        {
            $where.=' and pz.pz_time<='.strtotime($search_param['enddate']);
        }
        if(!empty($search_param['status']))
        {
            if(intval($search_param['status'])==4)
        ...
        $this->assign('peizistatus',$peizistatus);
        $this->assign('rows', $rows);
        $this->template('tuiguang_peizi_data.php');
    }
    ...
    public function tuiguang_fund() {
        ...
        $curpage = isset($_GET ['page']) ? intval($_GET ['page']) : 1;
        $pagesize = 15;
        $where = ' f.type=200 and f.uid=' . $this->uid;
        $time = time();

        $order = ' f.fund_id desc';
        $rows = \Model\Sys\Common::getTableRrcordList('user_fund_record f left join user_peizi pz on f.rec_id=pz.pz_id', "f.rtime,f.money,pz.pz_id,pz.manage_cost_day", $where, $order, $curpage, $pagesize);
        $rowcount = \Model\Sys\Common::getTableRecordCount('user_fund_record f left join user_peizi pz on f.rec_id=pz.pz_id', $where);
        $var['rowcount'] = $rowcount;
        $var['pagesize'] = $pagesize;
        $this->assign('var',$var);
    ...
    public function tuiguang_fund_data() {
        ...
        $this->assign('rows', $rows);
        $this->template('tuiguang_fund_data.php');
    }
    ...
    public function assets() {
        $this->setTitle('帐户资产—' . SITE_TITLE);
        $uid = $this->uid;
        $user = $this->user;
        $user ['user_name'] = substr_replace($user ['mobile'], '****', 3, 4);
        $user ['card_info'] = substr_replace($user ['id_card'], '****', 10, 4);
        $this->assign('userinfo', $user);
        $this->template('assets.php');
    }
