...
class Captcha
{
     ...
     */
    public function check($code, $id = '',$store_type = 'session')
    {
        $key = $this->authcode($this->seKey) . $id;
        // 验证码不能为空
        
        if(!isset($_SESSION)){ session_start();}
        $secode = $_SESSION[$key] ;//Session::get($key, '');
        ...
        if (time() - $secode['verify_time'] > $this->expire) {
            ...
            if($store_type == 'session'){
                if(!isset($_SESSION)){ session_start();}
                $_SESSION[$key] = null;//Session::delete($key, '');
            }
            else{
                cache('verify_'.$id, null);
            }
            return false;
        }
        ...
        if ($this->authcode(strtoupper($code)) == $secode['verify_code']) {
            ...
            if($store_type == 'session'){
                ...
                $this->reset && cache('verify_'.$id, null);
            }
            return true;
        }
     ...
     */
    public function entry($id = '',$store_type = 'session')
    {
        // 图片宽(px)
        ...
        // 绘验证码
        $code   = []; // 验证码
        $codeNX = ($this->imageW - $this->length*$this->fontSize*0.9)/2; // 验证码第1个字符的左边距
        $rotateAngle = 0;
        $offset_y = 0;
        if($this->compact){
            $offset_y =($this->imageH - $this->fontSize)/2+$this->fontSize;//居中
            //$offset_y = $this->fontSize * 1.1;
        }
        else{
            $offset_y =($this->imageH - $this->fontSize)/2+$this->fontSize;//居中
            //$offset_y = $this->fontSize * 1.6;
        }
        if($this->rotate){
            $rotateAngle = mt_rand(-40, 40);
        }
        else{
            $rotateAngle = 0;
        }
        if ($this->useZh) {
            ...
            for ($i = 0; $i < $this->length; $i++) {
                ...
                imagettftext($this->_image, $this->fontSize, $rotateAngle, $this->fontSize * ($i + 1) * 1.5, $this->fontSize + mt_rand(10, 20), $this->_color, $this->fontttf, $code[$i]);
            }
        } else {
            for ($i = 0; $i < $this->length; $i++) {
                ...
                imagettftext($this->_image, $this->fontSize, $rotateAngle, $codeNX, $offset_y, $this->_color, $this->fontttf, $code[$i]);
                if($this->compact){
                    $codeNX += $this->fontSize * 0.9;
                }
            }
        }
        // 保存验证码
        ...
        if($store_type == 'session'){
            ...
            cache('verify_'.$id, $secode,600);//添加到缓存，有效时间10分钟
        }
